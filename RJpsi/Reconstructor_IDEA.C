...
iFinalStates FindFinalStatesIndex(TClonesArray* branchTrack, Int_t nMu = 3) {
    ...
    for (int it = 0; it < nTracks; it++) {
        ...
        if (track1->PID == -13 && Length(particle1Obj->X, particle1Obj->Y, particle1Obj->Z) > 0) {
            vert[iLoc] = it;
            iLoc += 1;
        }
    }
    ...
    for (int iar = 0; iar < iLoc; iar++) {
        ...
        for (int it2 = 0; it2 < nTracks; it2++) {
            ...
            //  cout << track1->X << "; " << track2->X << endl;
            //  if (track1->X == track2->X) cout << "///" << endl;
            //  cout << track1->Y << "; " << track2->Y << endl;
            //  cout << track1->Z << "; " << track2->Z << endl;
            //  cout << track1->Charge + track2->Charge << endl;
            GenParticle* particle2Obj = (GenParticle*)track2->Particle.GetObject();
            // cout << particle1Obj->X << "; " << particle2Obj->X << endl;
            // cout << track2->X << "; " << particle2Obj->X << endl;
            //  same vertex
            //  if (track2->PID == 13 && track1->X == track2->X && track1->Y == track2->Y && track1->Z == track2->Z && track1->Charge + track2->Charge == 0) {
            if (track2->PID == 13 && particle1Obj->X == particle2Obj->X && particle1Obj->Y == particle2Obj->Y && particle1Obj->Z == particle2Obj->Z && particle1Obj->Charge + particle2Obj->Charge == 0) {
                // cout << "..." << endl;
...
vector<Int_t> findMisIDPi(iFinalStates iFS, TClonesArray* branchTrack) {
    ...
    for (int it = 0; it < nTracks; it++) {
        ...
        GenParticle* particleObj = (GenParticle*)track->Particle.GetObject();
        if (particleObj->X == 0 && particleObj->Y == 0 && particleObj->Z == 0) continue;
        iPiS.push_back(it);
    }
    return iPiS;
}
