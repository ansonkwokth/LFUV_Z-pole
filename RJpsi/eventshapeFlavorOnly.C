...
class vertexLength {
public:
    Float_t lengthFromPV;
    Int_t type;  // 0: others; 1: B meson; 2: C meson
};

//{{{
FWMoments
updateFWMs(Float_t E1, Float_t E2, Float_t cosine_theta, FWMoments FWMs) {
    FWMoments FWMs_;
...
void eventshapeFlavorOnly(const string type, const Bool_t save = false, Int_t num_test = 0, bool printOut = false) {
    ...
    Float_t BMesonLegth = 0;
    Int_t NBMeson = 0;
    Float_t BMaxLength = -99999;
    Float_t BMinLength = 99999;
    Float_t CMesonLegth = 0;
    Int_t NCMeson = 0;
    Float_t CMaxLength = -99999;
    Float_t CMinLength = 99999;

    Float_t otherLength = 0;
    Int_t Nother = 0;
    Float_t otherMaxLength = -99999;
    Float_t otherMinLength = 99999;
    ...
    // TFile leg(lengthOutput, "recreate");
    // TTree leg_tr("s", "length");
    // vertexLength* vleg = new vertexLength;
    // leg_tr.Branch("vertexLength", &vleg);
    ...
    for (Int_t i_en = 0; i_en < numberOfEntries; i_en++) {
        ...
        nEvt += 1;
        Float_t tracksEnergy = 0;
        // cout << "---------------------Event: " << i_en << endl;

        Int_t nDisplacedTr = 0;
        FWMoments FWMs_;
        // 1st tracks loop
        for (Int_t itr1 = 0; itr1 < nTracks; itr1++) {
            track1 = (Track*)branchTrack->At(itr1);

            // get the stat of the length of the typical b/c meson and others
            GenParticle* particleTr = (GenParticle*)track1->Particle.GetObject();
            ...
            if (length(track1->X, track1->Y, track1->Z) < 0.1) continue;  // lower cut
            if (length(track1->X, track1->Y, track1->Z) > 20) continue;   // upper cut
            nDisplacedTr += 1;
            TLorentzVector cp1;
            ...
            for (Int_t itr2 = 0; itr2 < nTracks; itr2++) {
                ...
                if (length(track2->X, track2->Y, track2->Z) < 0.1) continue;
                if (length(track2->X, track2->Y, track2->Z) > 20) continue;
                TLorentzVector cp2;
        ...
        // FWMs->H_EE0 = FWMs_.H_EE0 / pow(tracksEnergy, 2);
        // FWMs->H_EE1 = FWMs_.H_EE1 / pow(tracksEnergy, 2);
        // FWMs->H_EE2 = FWMs_.H_EE2 / pow(tracksEnergy, 2);
        //// cout << " FWMs: " << FWMs->H_EE2 << "\n";
        // FWMs->H_EE3 = FWMs_.H_EE3 / pow(tracksEnergy, 2);
        // FWMs->H_EE4 = FWMs_.H_EE4 / pow(tracksEnergy, 2);
        // FWMs->H_EE5 = FWMs_.H_EE5 / pow(tracksEnergy, 2);
        // FWMs->H_EE6 = FWMs_.H_EE6 / pow(tracksEnergy, 2);
        // FWMs->H_EE7 = FWMs_.H_EE7 / pow(tracksEnergy, 2);
        // FWMs->H_EE8 = FWMs_.H_EE8 / pow(tracksEnergy, 2);
        // FWMs->H_EE9 = FWMs_.H_EE9 / pow(tracksEnergy, 2);
        // FWMs->H_EE10 = FWMs_.H_EE10 / pow(tracksEnergy, 2);
        tr.Fill();
    }
    tr.Write();
    fea.Close();
