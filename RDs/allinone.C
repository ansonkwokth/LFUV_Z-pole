...
Float_t
Length(Float_t X, Float_t Y, Float_t Z) {
...
//----------{{{
void distance_2lines(Float_t X1, Float_t Y1, Float_t Z1, Float_t Px1, Float_t Py1, Float_t Pz1, Float_t X2, Float_t Y2, Float_t Z2, Float_t Px2, Float_t Py2, Float_t Pz2, Float_t* L, Float_t* s1, Float_t* s2) {
    ...
    Float_t A = (dx * Px2 + dy * Py2 + dz * Pz2) / (P2 * P2) *
                (Px1 * Px2 + Py1 * Py2 + Pz1 * Pz2);
    Float_t B = (dx * Px1 + dy * Py1 + dz * Pz1);
    Float_t C =
        pow((Px1 * Px2 + Py1 * Py2 + Pz1 * Pz2), 2) / (P1 * P1 * P2 * P2);
    ...
    Float_t L_ = pow((dx + s1_ * Px1 - s2_ * Px2), 2) +
                 pow((dy + s1_ * Py1 - s2_ * Py2), 2) +
                 pow((dz + s1_ * Pz1 - s2_ * Pz2), 2);
...
distance_linepoint(TVector3 v3Point, TVector3 v3Line, TLorentzVector line) {
    Float_t proj = (line.Px() * (v3Point.X() - v3Line.X()) +
                    line.Py() * (v3Point.Y() - v3Line.Y()) +
                    line.Pz() * (v3Point.Z() - v3Line.Z())) /
                   (line.P() * line.P());
...
public:
    ...
    Float_t ENeutral03;        // sum of neutral particle energ, which has angle
                               // between B < 0.3rad
    Float_t ENeutral06;        // sum of neutral particle energ, which has angle
                               // between B < 0.6rad
    Float_t ENeutral03Hadron;  // sum of neutral hadron energ, which has angle
                               // between B < 0.3rad
    Float_t ENeutral06Hadron;  // sum of neutral hadron energ, which has angle
                               // between B < 0.6rad
    Float_t ENeutral03Photon;  // sum of photon energ, which has angle between B
                               // < 0.6rad
    Float_t ENeutral06Photon;  // sum of photon energ, which has angle between B
                               // < 0.6rad
    Float_t ECharge03;         // sum of charged  particle energ, which has angle
                               // between B < 0.6rad
    Float_t ECharge06;         // sum of charged particle energ, which has angle
                               // between B < 0.6rad
    Float_t ECharge03PV;       // sum of chaged PV particle energ, which has angle
                               // between B < 0.6rad
    Float_t ECharge06PV;       // sum of charged PV particle energ, which has angle
                               // between B < 0.6rad
    Float_t ECharge03DV;       // sum of charged DV particle energ, which has angle
                               // between B < 0.6rad
    Float_t ECharge06DV;       // sum of charged DV particle energ, which has angle
                               // between B < 0.6rad
...
Angle(Float_t X1, Float_t Y1, Float_t Z1, Float_t X2, Float_t Y2, Float_t Z2) {
    Float_t angle = acos((X1 * X2 + Y1 * Y2 + Z1 * Z2) /
                         (Length(X1, Y1, Z1) * Length(X2, Y2, Z2)));
    return angle;
}
//}}}
