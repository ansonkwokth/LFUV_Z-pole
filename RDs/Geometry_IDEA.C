...
Float_t closestTrack(iFinalStates iFS, TLorentzVector target, TVector3 v3Target, double noise, TClonesArray* branchTrack) {
    ...
    for (int it = 0; it < nTracks; it++) {
        ...
        // dispalced vertex
        GenParticle* particleOtherObj = (GenParticle*)trackOther->Particle.GetObject();
        // if (trackOther->X == 0 && trackOther->Y == 0 && trackOther->Z == 0) continue;
        if (particleOtherObj->X == 0 && particleOtherObj->Y == 0 && particleOtherObj->Z == 0) continue;
        ...
        Float_t XTr = particleOtherObj->X + distribution(genertator);
        Float_t YTr = particleOtherObj->Y + distribution(genertator);
        Float_t ZTr = particleOtherObj->Z + distribution(genertator);
