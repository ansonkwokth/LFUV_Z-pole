...
iFinalStates FindFinalStatesIndex(TClonesArray* branchTrack) {
    ...
    for (int it = 0; it < nTracks; it++) {
        ...
        if (track1->PID == 321 && Length(particle1Obj->X, particle1Obj->Y, particle1Obj->Z) > 0) {
            vert[iLoc] = it;
            iLoc += 1;
            // cout << "---1" << endl;
        }
    ...
    for (int iar = 0; iar < iLoc; iar++) {
        ...
        for (int it2 = 0; it2 < nTracks; it2++) {
            ...
            // cout << particle1Obj->X << "; " << particle1Obj->Y << "; " << particle1Obj->Z << "; " << endl;
            // cout << particle2Obj->X << "; " << particle2Obj->Y << "; " << particle2Obj->Z << "; " << endl;
            if (track2->PID == -321 &&
                ...
                for (int it3 = 0; it3 < nTracks; it3++) {
                    ...
                    if (abs(particle3Obj->PID) == 211 && particle1Obj->X == particle3Obj->X && particle1Obj->Y == particle3Obj->Y && particle1Obj->Z == particle3Obj->Z) {
                        iKPos = vert[iar];
                        iKNeg = it2;
                        iPi = it3;
                        foundK = 1;
